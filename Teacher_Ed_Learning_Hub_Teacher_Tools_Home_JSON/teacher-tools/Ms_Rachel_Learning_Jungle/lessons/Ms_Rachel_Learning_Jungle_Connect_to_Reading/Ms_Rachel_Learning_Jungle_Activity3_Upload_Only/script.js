...
// Activity 3 — picture upload feature only
function resizeSchoolImage(file,maxSize=1200,quality=.82){
  return new Promise((resolve,reject)=>{
    ...
    reader.onload=()=>{
      ...
      img.onload=()=>{
        ...
        if(w>maxSize||h>maxSize){const scale=Math.min(maxSize/w,maxSize/h);w=Math.round(w*scale);h=Math.round(h*scale);}
...
async function uploadSchoolImage(input,number){
  const file=input.files&&input.files[0]; if(!file)return;
  if(!file.type.startsWith("image/")){alert("Please choose an image file.");input.value="";return;}
  try{
    const dataUrl=await resizeSchoolImage(file);
    localStorage.setItem(`lesson2SchoolImage${number}`,dataUrl);
    showSchoolImage(number,dataUrl);
  }catch(e){alert("The picture could not be loaded. Please try another image.");}
  input.value="";
}
function showSchoolImage(number,dataUrl){
  const slot=$(`#pictureSlot${number}`); if(!slot)return;
  slot.classList.add("has-image");
  slot.innerHTML=`
    <img src="${dataUrl}" alt="School place picture ${number}">
    <div class="picture-upload-actions">
      <button type="button" class="upload-picture-btn" onclick="document.getElementById('schoolImage${number}').click()">🔄 Replace</button>
      <button type="button" class="remove-picture-btn" onclick="removeSchoolImage(${number})">✕ Remove</button>
    </div>
    <input type="file" id="schoolImage${number}" accept="image/*" hidden onchange="uploadSchoolImage(this,${number})">
  `;
}
function removeSchoolImage(number){
  localStorage.removeItem(`lesson2SchoolImage${number}`);
  const slot=$(`#pictureSlot${number}`); if(!slot)return;
  slot.classList.remove("has-image");
  slot.innerHTML=`
    <span>🖼️</span><small>ADD SCHOOL PICTURE ${number}</small>
    <input type="file" id="schoolImage${number}" accept="image/*" hidden onchange="uploadSchoolImage(this,${number})">
    <button type="button" class="upload-picture-btn" onclick="document.getElementById('schoolImage${number}').click()">📷 Upload Picture</button>
  `;
}
function loadSchoolImages(){
  for(let i=1;i<=5;i++){
    const dataUrl=localStorage.getItem(`lesson2SchoolImage${i}`);
    if(dataUrl)showSchoolImage(i,dataUrl);
  }
}
loadSchoolImages();
